...
// verify digit
	...
	private static HashMap<String, Integer> dateMap;	//日期Map集合
	private static HashMap<String, String> areaCodeMap; //地区代码集合
	static {
	...
	public boolean verifyAreaCode(String code) {
		String areaCode = code.substring(0, 2); //获取省份代码，如江苏：320
	...
	public boolean verifyBirthdayCode(String code) {
		...
		boolean isEighteenCode = (18 == code.length()); //是否已满18周岁
		...
		Integer day = dateMap.get(month);			//出生日期
		String yearCode = code.substring(6, 10);
		Integer year = Integer.valueOf(yearCode);	//出生年份
	 ...
	 * 15位身份证码转18位身份证码
	 *
	 * @param fifteencardid 15位身份证码ID
	 * @return 18位身份证码 string
	 */
	 ...
	 * @return boolean boolean
	 */
	public static boolean verifyIDCard(String cardId)
	{
		IDCardValidateUtil card = new IDCardValidateUtil();
		return card.verify(cardId);
	}
}
